...
TRANSLATIONS = {
    ...
    'menu_unknown_command':{'en': 'Unknown command.', 'ru': 'Неизвестная команда.', 'zh': '未知的 команда。'},
    ...
    'news_photos_prompt': {'en': 'Attach up to 10 photos for the news.', 'ru': 'Прикрепите до 10 фото для новости.', 'zh': '为新闻附上最多10张图片。'},
    ...
    'news_edit_prompt': {'en': 'Send the new text for the news:', 'ru': 'Отправьте новый текст для новости:', 'zh': '发送新闻的新文本：'},
    'done':             {'en': 'Operation completed. Data updated.', 'ru': 'Операция завершена. Данные обновлены.', 'zh': '操作完成。数据已更新。'},
    ...
    'clean_prompt':     {'en': 'What would you like to clear?', 'ru': 'Что вы хотите стереть?', 'zh': '您想清除什么？'},
    'clean_confirm_all':{'en': 'Yes, clear all', 'ru': 'Да, стереть все данные', 'zh': '是的，全部清除'},
    'clean_confirm_all_prompt': {'en': 'Confirm clearing ALL data (time/salary/cash) for ALL groups?', 'ru': 'Подтвердите удаление ВСЕХ данных (время/зарплата/наличные) по ВСЕМ группам?', 'zh': '确认清除所有群组的所有数据（时间/工资/现金）？'},
    'clean_section_prompt': {'en': 'You chose: {section}\nSelect a group or "Clear all {section}"', 'ru': 'Вы выбрали: {section}\nВыберите группу или «Стереть все {section}»', 'zh': '您已选择：{section}\n请选择一个群组或“清除所有{section}”'},
    'clean_confirm_section_prompt': {'en': 'Confirm clearing {section} for ALL groups?', 'ru': 'Подтвердите удаление {section} по ВСЕМ группам?', 'zh': '确认清除所有群组的{section}？'},
    'clean_confirm_group': {'en': 'Yes, clear', 'ru': 'Да, стереть!', 'zh': '是的，清除！'},
    'clean_group_prompt': {'en': 'Confirm clearing {section} for group {group}', 'ru': 'Подтвердите удаление {section} для группы {group}', 'zh': '确认清除群组{group}的{section}？'},
    'clean_done_all': {'en': 'Cleared all {section} for all groups.', 'ru': 'Все данные «{section}» стёрты для всех групп.', 'zh': '已为所有群组清除所有{section}。'},
    'clean_done_group': {'en': 'Cleared {section} for group {group}.', 'ru': 'Данные «{section}» стёрты для группы {group}.', 'zh': '已为群组{group}清除{section}。'},
    ...
    'money_amount_prompt': {'en': 'Please enter the amount for group {group}:', 'ru': 'Введите сумму для группы {group}:', 'zh': '请输入群组 {group} 的金额：'},

    # Salary admin
    'salary_choose_group': {'en': 'Choose a group to configure salary:', 'ru': 'Выберите группу для настройки зарплаты:', 'zh': '请选择要配置工资的组：'},
    'salary_option_prompt': {'en': 'Group: <b>{group}</b>\nCurrent option: <b>{current}</b>\nSelect a new one:', 'ru': 'Группа: <b>{group}</b>\nТекущая опция: <b>{current}</b>\nВыберите новую:', 'zh': '组：<b>{group}</b>\n当前选项：<b>{current}</b>\n请选择新的：'},
    'salary_set': {'en': 'Salary option for {group} set to {opt}.', 'ru': 'Опция зарплаты для {group} установлена: {opt}.', 'zh': '已为组{group}设置工资选项：{opt}。'},

    # ID Photo
    'photo_id': {'en': 'file_id of your photo:\n<code>{file_id}</code>', 'ru': 'file_id вашего фото:\n<code>{file_id}</code>', 'zh': '您照片的 file_id：\n<code>{file_id}</code>'},
    'no_photo': {'en': 'You did not attach a photo to the /id command.', 'ru': 'Вы не прикрепили фото к команде /id.', 'zh': '您未在 /id 命令中附加图片。'},

    # Emoji
    'emoji_choose_user': {'en': 'Select a user:', 'ru': 'Выберите пользователя:', 'zh': '选择用户：'},
    'emoji_choose_emoji': {'en': 'Select emoji for user {target_id}:', 'ru': 'Выберите эмоджи для пользователя {target_id}:', 'zh': '为用户 {target_id} 选择表情：'},
    'emoji_assigned': {'en': 'Emoji assigned to user {target_id}: {emoji}', 'ru': 'Пользователю {target_id} присвоен эмоджи: {emoji}', 'zh': '用户 {target_id} 已分配表情：{emoji}'},
    ...
    'emoji_notify': {'en': 'You have been assigned emoji: {emoji}\nAll commands are now available!', 'ru': 'Вам назначен эмоджи: {emoji}\nТеперь доступны все команды!', 'zh': '您已分配表情：{emoji}\n现在所有命令均可用！'},

...
async def cmd_clean(cb: CallbackQuery, state: FSMContext):
    """
    Инициализация процесса очистки (вызывается при нажатии кнопки Clean в админ-меню)
    """
    ...
    sent = await cb.message.answer(
        get_message(lang, "clean_prompt"), parse_mode="HTML", reply_markup=kb
    )
...
# последующие стадии очистки без проверок is_user_admin
...
async def process_clean_menu(cb: CallbackQuery, state: FSMContext):
    ...
    if choice == "all":
        ...
        await cb.message.edit_text(
            get_message(lang, "clean_confirm_all_prompt"), parse_mode="HTML", reply_markup=kb
        )
...
async def confirm_all_section(cb: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[CLEAN] Confirm all: section={section}")
    for grp in groups_data:
...
async def process_section_group_choice(cb: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=get_message(lang, "clean_confirm_group"), callback_data=f"confirm_grp_{section}_{grp}" )],
        [InlineKeyboardButton(text=get_message(lang, "btn_cancel"), callback_data="clean_cancel")]
    ])
    await cb.message.edit_text(
...
async def confirm_group_section(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await cb.answer(get_message(lang, "clean_done_group", section=get_message(lang, f"clean_{section}"), group=grp))
    await state.clear()

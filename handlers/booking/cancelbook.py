...
async def cmd_off(message: Message):
    ...
    if not rows:
        return await message.answer(get_message(lang, "no_active_bookings"))

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=f"❌ {r['day']} {r['time_slot']} ({r['group_key']})",
                                   callback_data=f"off_cancel_user_{r['id']}")]
            for r in rows
        ]
    )
    await message.answer(get_message(lang, "off_choose_booking"), reply_markup=kb)
...
async def off_cancel_user(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with db.db_pool.acquire() as conn:
        ...
        if not row:
            return await callback_query.answer(get_message(lang, "no_such_booking"), show_alert=True)

        gk, day, slot = row.values()
        await conn.execute("DELETE FROM bookings WHERE id=$1", bid)
        await conn.execute("DELETE FROM group_time_slot_statuses WHERE group_key=$1 AND day=$2 AND time_slot=$3",
                           gk, day, slot)
    ...
    await callback_query.answer(get_message(lang, "booking_cancelled"))
    await callback_query.message.edit_text(get_message(lang, "booking_cancelled"))
...
async def off_cancel_admin(callback_query: CallbackQuery, state: FSMContext):
    ...
    await update_group_message(callback_query.bot, gk)
    await callback_query.answer(get_message(lang, "booking_cancelled_by_admin"))
    await callback_query.message.edit_text(get_message(lang, "booking_cancelled_by_admin"))
